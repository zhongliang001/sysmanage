...
public class RoleRightControllerTest {

    private Logger logger = LoggerFactory.getLogger(RoleRightControllerTest.class);
...
//                .andExpect(MockMvcResultMatchers.status().isOk()) //400错误请求
        ...
        RoleMenuRightDto roleMenuRightDto = new RoleMenuRightDto();
        roleMenuRightDto.setMenuRightDtos(list);
        roleMenuRightDto.setRoleId("000009");
        String requestBody2 =mapper.writeValueAsString(roleMenuRightDto);
        MvcResult result2 = mockMvc.perform(MockMvcRequestBuilders
                .post("/roleright/addAction").contentType(MediaType.APPLICATION_JSON_VALUE).content(requestBody2)
                // 设置返回值类型为utf-8，否则默认为ISO-8859-1
                .accept(MediaType.APPLICATION_JSON_UTF8))
//                .param("name", "Tom"))
                .andExpect(MockMvcResultMatchers.status().isOk())
